...
async function run(client, message, args) {
    ...
    if (user.hasPermission('ADMINISTRATOR'))
        return await message.channel.send(new discord_js_1.MessageEmbed().setDescription(`${emojis_json_1.error} You can't softban a Moderator.`));
    let reason;
    ...
    user.ban({ reason: `${reason}`, days: 7 }).then(() => {
        var _a;
        (_a = message.guild) === null || _a === void 0 ? void 0 : _a.members.unban(user);
    });
    user.send(embed);
}
exports.run = run;

...
async function run(client, message, args) {
    ...
    if (user.roles.cache.has(`${role}`))
        return await message.channel.send(new discord_js_1.MessageEmbed().setDescription(`${emojis_json_1.error} This user has already the role ${role.id}.`));
    user.roles.add(`${role}`);
    message.channel.send(new discord_js_1.MessageEmbed().setDescription(`${emojis_json_1.tick} The role has been added to ${user.toString()}`));
}
exports.run = run;
exports.help = {
    aliases: ['addrole'],
